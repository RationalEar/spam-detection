...
class BiLSTMLimeExplainer:
    ...
    def predict_proba(self, texts):
        ...
        with torch.no_grad():
            ...
            # Handle the case when outputs is a tuple (common in many PyTorch models)
            # Extract the main prediction tensor (typically the first element in the tuple)
            ...
            if len(prediction_tensor.shape) > 1:
                # If multi-dimensional, squeeze to 1D
                spam_probs = prediction_tensor.squeeze().cpu().numpy()
            else:
                # If already 1D, use as is
                spam_probs = prediction_tensor.cpu().numpy()
...
def get_lime_explanation(model, original_text, word_to_idx, idx_to_word, num_features=10, num_samples=500, alpha=0.01):
    ...
    explainer_wrapper = BiLSTMLimeExplainer(model, word_to_idx, idx_to_word, str(device))
